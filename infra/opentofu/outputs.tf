...
output "fly_env" {
  ...
  value = {
    ...
    NIKS3_OIDC_CONFIG       = "/run/secrets/niks3-oidc-config"
...
output "oidc_config_json" {
  description = "OIDC configuration JSON written to the Fly guest as a secret file."
  value       = local.oidc_config_json
}
